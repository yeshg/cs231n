...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    scores = X[i].dot(W) #X[i] gives us single image (1,3073), we dot this with W (3073,10) to get scores (10,1)
  ...
  # Add regularization to the loss, to make W matricies unique and dependent on magnitude of elements
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  dW = X.T.dot(X_mask)
  # print("X dim: {0}    X_mask dim: {1}    dW dim: {2}").format(X.shape, X_mask.shape, dW.shape)
